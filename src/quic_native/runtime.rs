...
const QUIC_CONNECTION_POOL_SELECTION_POLICY: &str = "least-loaded by active streams, stream open failures, latency, backpressure, resets, and control health";
...
impl StateSlot {
    ...
    pub async fn status_value(&self) -> serde_json::Value {
        ...
        status.insert(
            ...
            serde_json::to_value(self.last_error.lock().await.clone())
                .expect("control error serializable"),
        );
        status.insert(
        ...
        status.insert(
            ...
            serde_json::json!({
                ...
                "pool": {
                    ...
                    "policy": "disconnected",
                    "workload_hint": serde_json::Value::Null,
                ...
                "copy": {
                    "buffer_size": QUIC_NATIVE_COPY_BUFFER_SIZE,
                    "duration_samples": 0,
                    "failures": 0,
                    "backpressure_timeouts": 0,
                },
        ...
        status.insert(
            ...
            serde_json::json!({
                "health": {
                    ...
                    "pool_workload_hint": serde_json::Value::Null,
                    ...
                    "open_successes": 0,
                    "open_failures": 0,
                    "open_latency_ms": serde_json::Value::Null,
                    ...
                    "last_close_reason": serde_json::Value::Null,
...
fn route_id(args: &cli::ProxyArgs) -> String {
    format!("quic-native:{}:{}", args.listen, args.remote_name)
}

fn quic_options_from_proxy_args(
    args: &cli::ProxyArgs,
...
fn quic_connection_pool_policy(args: &cli::ProxyArgs) -> String {
    args.pool_policy
        .clone()
        .unwrap_or_else(|| "explicit".to_string())
}

fn quic_connection_pool_workload_hint(args: &cli::ProxyArgs) -> Option<&'static str> {
...
fn quic_connection_pool_reason(args: &cli::ProxyArgs) -> String {
...
fn quic_connection_pool_mode(pool_size: usize) -> &'static str {
    if pool_size > 1 {
        "workload-aware-pool"
    } else {
        "single-connection"
...
fn insert_zero_quic_lifecycle_status(status: &mut serde_json::Map<String, serde_json::Value>) {
    for key in [
        "quic_stream_open_samples",
        "quic_stream_open_failures",
        "quic_header_write_samples",
    ...
    for key in [
        ...
        status.insert(key.to_string(), serde_json::Value::Null);
    }
}

fn quic_profile_next_bottleneck(
...
impl Drop for Stream {
    fn drop(&mut self) {
        if !self.closed.swap(true, Ordering::Relaxed) {
            self.inner
                .reset(quinn::VarInt::from_u32(super::FLOW_RESET_ERROR_CODE));
