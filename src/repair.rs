...
pub(crate) fn action_for_blocker(blocker: &str) -> Option<RepairAction> {
    match blocker {
...
pub(crate) fn action_value_for_blocker(blocker: &str) -> Value {
    action_for_blocker(blocker)
        .and_then(|action| serde_json::to_value(action).ok())
        .unwrap_or(Value::Null)
}

pub(crate) fn attach_repair_action(object: &mut serde_json::Map<String, Value>, blocker: &str) {
